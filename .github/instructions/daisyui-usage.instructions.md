...
3. If customization of daisyUI styles using Tailwind CSS utility classes didn't work because of CSS specificity issues, you can use the `!` prefix on the Tailwind CSS utility class to override the existing styles. For example `btn !bg-red-500` sets a custom background color to a `btn` forcefully. This is a last resort solution and should be used sparingly
